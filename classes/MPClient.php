...
class MPClient {
	 ...
	 */
    public static function register($empresa, $cnpj, $endereco, $complemento, $cidade, $uf, $telefone, $celular, $nome, $email, $senha, $subdomain) {
		...
		$query = $conn->prepare('insert into clientes 
			...
			values (:empresa, :cnpj, :endereco, :complemento, :cidade, :uf, :telefone, :celular, :nome, :email, :senha, :subdominio)');
	...
	/**
	 * Verifica se login e senha são válidos para o cliente
	 */
	public static function checkLogin($email, $senha) {
		$conn = MPDatabase::db();
		$query = $conn->prepare('select count(*) from clientes where email = ? and senha = ?');
	...
	public static function getClient($id) {
		$coluna = 'id';
		if(is_string($id)) $coluna = 'email';
		
		$conn = MPDatabase::db();
		$query = $conn->prepare('select * from clientes where '.$coluna.' = ? limit 1');
		$query->execute(array($id));
		return $query->fetch();
	}
	
	/**
	 * Retorna um cliente TESTE para fazer testes
	 */
	public static function getTestClient() {
		$conn = MPDatabase::db();
		$query = $conn->query('select * from clientes limit 1');
		return $query->fetch();
	}
	 ...
	 */
	public static function checkToken($aplicativo, $object_id, $token, $object_type = 'usuario')
	{
		$conn = MPDatabase::db();
		$query = $conn->prepare('select * from logados
					where aplicativo = ?
					and object_type = ?
					and object_id = ?
					and token = ?
					and created_at >= date_sub(NOW(), interval 24 hour)');
		$query->execute(array($aplicativo, $object_type, $object_id, $token));
		return $query->fetch();
	}
	/**
	 * Cadastra um novo token
	 */
	public static function setToken($aplicativo, $object_id, $token, $object_type = 'usuario')
	{
		$conn = MPDatabase::db();
		$query = $conn->prepare('insert into logados
					(aplicativo, object_type, object_id, token)
					values
					(?, ?, ?, ?)');
		return $query->execute(array($aplicativo, $object_type, $object_id, $token));
	}
